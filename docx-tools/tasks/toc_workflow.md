...
```bash
python scripts/internal_nav.py files/input.docx --out files/with_static_toc.docx
```
...
```bash
python scripts/insert_toc.py files/input.docx --out files/with_toc.docx
```
...
```bash
python render_docx.py files/with_toc.docx --output_dir files/out
```
