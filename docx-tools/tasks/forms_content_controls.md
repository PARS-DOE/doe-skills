...
```bash
python scripts/content_controls.py files/template.docx wrap_placeholders \
  --output files/template_sdt.docx
```

3. **Populate SDTs by tag**
```bash
python scripts/content_controls.py files/template_sdt.docx fill \
  --set NAME="Ada Lovelace" \
  --set EMAIL="ada@example.com" \
  --output files/filled.docx
```

4. **Render for QA**
```bash
python render_docx.py files/filled.docx --output_dir files/out_forms
```
...
```bash
python scripts/content_controls.py files/template_sdt.docx list --json
```
