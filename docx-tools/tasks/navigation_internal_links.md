...
```bash
python scripts/captions_and_crossrefs.py files/in.docx files/with_caps.docx --figures --tables --bookmarks
```

3) Add navigation

```bash
python scripts/internal_nav.py files/with_caps.docx --out files/with_nav.docx
```

4) Render and verify

```bash
python render_docx.py files/with_nav.docx --output_dir files/out_nav
```
