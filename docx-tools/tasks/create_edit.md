...
### 4) Tracked changes require docx-revisions
If the user requests *real* tracked changes (redlines), use the `docx-revisions` library — see `tasks/edit_with_tracked_changes.md` for the full workflow. For *real* Word comments, use OOXML patching (see `ooxml/comments.md`).
...
## Output hygiene
Keep `files` clean: deliverables only unless the user asks for intermediate render artifacts.
