...
python scripts/fields_materialize.py \
  files/with_refs.docx \
  --out files/with_refs_materialized.docx
```
...
```bash
python scripts/fields_materialize.py files/with_refs.docx --out files/out.docx --only REF
```

### 4) Render and visually QA
```bash
python render_docx.py files/with_refs_materialized.docx --output_dir files/out_caps
```
Inspect the PNGs.
