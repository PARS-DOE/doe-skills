...
```bash
python render_docx.py files/input.docx --output_dir files/out
# For debugging LibreOffice failures:
python render_docx.py files/input.docx --output_dir files/out --verbose
# Optional: also write <input_stem>.pdf to --output_dir (for debugging/archival):
python render_docx.py files/input.docx --output_dir files/out --emit_pdf
```
...
LO_PROFILE=files/.lo_profile_${BASENAME}_$$
...
- `files` contains only final outputs (unless user asked for intermediates)
