---
name: docx-tools
description: "Word document creation, editing, and analysis with python-docx and LibreOffice. Use when working with .docx files: creating professional documents, editing content, tracked changes, comments, redaction, accessibility audits, style normalization, or visual rendering and QA."
---

# DOCX Skill (Read • Create • Edit • Redline • Comment)

Use this skill when you need to create or modify `.docx` files in the workspace and verify them visually.
...
- Run `skills/docx-tools/render_docx.py` to produce `page-<N>.png` images (optionally also a PDF with `--emit_pdf`)
- Visually inspect the PNGs using `mastra_workspace_read_file` with `encoding: "base64"` — you have multimodal vision and can see them directly
...
### Tracked changes (redlines) — use docx-revisions, NOT python-docx

When the user wants edits visible as Word tracked changes (redlines, “Track Changes on”), **do not use python-docx** — it has no tracked changes support. Use `docx-revisions` instead:

**CLI (simple find-and-replace):**
```bash
python skills/docx-tools/scripts/add_tracked_replacements.py input.docx --out output.docx \
  --replace “old text=new text” \
  --delete “paragraph text to remove” \
  --insert “new paragraph to add” \
  --author “Walt”
```

**Python (complex or conditional edits):**
```python
...
# Enable trackRevisions so Word shows tracking mode when opened
settings = rdoc.document.settings.element
if settings.find(qn(“w:trackRevisions”)) is None:
    settings.append(OxmlElement(“w:trackRevisions”))

# Find-and-replace (handles text split across multiple XML runs)
rdoc.find_and_replace_tracked(“old phrase”, “new phrase”, author=”Walt”)

# Delete a text range within a paragraph (by character offset)
...
For the full workflow (including accepting/rejecting changes), see `tasks/edit_with_tracked_changes.md`.

## Quick start (common one-liners)

```bash
# 1) Render any DOCX to PNGs (visual QA)
python skills/docx-tools/render_docx.py input.docx --output_dir out

# 2) Edit with tracked changes (redlines visible in Word)
python skills/docx-tools/scripts/add_tracked_replacements.py input.docx --out edited.docx \
  --replace "old text=new text" --author "Walt"

# 3) Accept tracked changes (finalization)
python skills/docx-tools/scripts/accept_tracked_changes.py input.docx --mode accept --out accepted.docx

# 4) Remove reviewer comments (finalization)
python skills/docx-tools/scripts/comments_strip.py input.docx --out no_comments.docx

# 5) Accessibility audit (+ optional safe fixes)
python skills/docx-tools/scripts/a11y_audit.py input.docx
python skills/docx-tools/scripts/a11y_audit.py input.docx --out_json a11y_report.json
python skills/docx-tools/scripts/a11y_audit.py input.docx --fix_image_alt from_filename --out a11y_fixed.docx

# 6) Redact sensitive text (layout-preserving by default)
python skills/docx-tools/scripts/redact_docx.py input.docx redacted.docx --emails --phones
```
...
4. **Only if needed**: use `docx-revisions` for tracked changes, OOXML patching for comments, hyperlinks, or fields.
...
```bash
python skills/docx-tools/render_docx.py files/input.docx --output_dir files/out
# If debugging LibreOffice:
python skills/docx-tools/render_docx.py files/input.docx --output_dir files/out --verbose
# Optional: also write <input_stem>.pdf to --output_dir (for debugging/archival):
python skills/docx-tools/render_docx.py files/input.docx --output_dir files/out --emit_pdf
```

Then inspect the generated `page-<N>.png` files using `mastra_workspace_read_file` with `encoding: "base64"` to view them visually.
...
- If you need **tracked changes (redlines)**: `tasks/edit_with_tracked_changes.md`
