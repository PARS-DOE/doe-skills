...
Usage
-----
python scripts/make_fixtures.py --outdir files/fixtures
python scripts/make_fixtures.py --outdir files/fixtures --only tracked
...
def make_tracked_changes_fixture(out_path: str) -> None:
    ...
    doc_xml_bytes = etree.tostring(doc_xml, xml_declaration=True, encoding="UTF-8", standalone="yes")
    settings_bytes = etree.tostring(settings, xml_declaration=True, encoding="UTF-8", standalone="yes")
...
def _inject_vml_watermark_into_header(header_xml: etree._Element, text: str) -> None:
    ...
    shape.set("style", "position:absolute;margin-left:0;margin-top:0;width:468pt;height:234pt;rotation:315")
...
def make_watermark_fixture(out_path: str) -> None:
    ...
    header_bytes = etree.tostring(header_xml, xml_declaration=True, encoding="UTF-8", standalone="yes")
