...
```bash
python skills/pdf-tools/scripts/render_pdf.py input.pdf --out_dir files/_renders/input
```
...
```bash
python skills/pdf-tools/scripts/render_pdf.py input.pdf --out_dir files/_renders/input --pages 1-5
```

### 2) Get a quick structural overview

```bash
python skills/pdf-tools/scripts/pdf_inspect.py input.pdf
```
...
```bash
python skills/pdf-tools/scripts/pdf_extract.py text input.pdf --method pdfplumber --pages 3-4 --out files/_tmp/extracted.txt
```
...
```bash
python skills/pdf-tools/scripts/ocr_pdf.py input.pdf -o files/_tmp/input_ocr.pdf
```
