...
```bash
python skills/pdf-tools/scripts/render_pdf.py report.pdf --out_dir files/_renders/report
```
...
```bash
python skills/pdf-tools/scripts/latex_to_pdf.py input.tex -o output.pdf
```
...
```bash
python skills/pdf-tools/scripts/md_to_pdf.py input.md -o output.pdf
```
...
```bash
python skills/pdf-tools/scripts/html_to_pdf.py input.html -o output.pdf
```
...
```bash
python skills/pdf-tools/scripts/lo_convert_to_pdf.py input.docx --out_dir files/_tmp
```
...
- `skills/docs` for DOCX
- `skills/slides` for PPTX
...
Finally, delete intermediate artifacts under `files/_tmp` and `files/_renders`.
